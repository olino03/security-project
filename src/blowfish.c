...
static void swap_uint(uint *a, uint *b) {
    uint t = *a;
    *a = *b;
    *b = t;
}

static uint F(const blowfish_ctx *ctx, uint x) {
    uint h = ctx->S[0][(x >> 24) & 0xFF] + ctx->S[1][(x >> 16) & 0xFF];
